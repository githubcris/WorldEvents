<!DOCTYPE html>
<%@page import="worldevents.Myevent"%>
...
<div class="divCenterClass">
  ...
  <g:if  test="${params.mode == 'detail'}" >  
        ...
        <label style="pad-left:10px;color:black;">  |  </label>
     	<g:link class="headLabel" controller="event"><g:message code="label.title.events"/></g:link>
   	     ...
   	     <p class="separator"></p>
   	     <g:if test="${session?.user?.twitter_id == myevent?.user?.twitter_id }">
   	     <div  class="label">
   	     <table class="tablemenu">
   	     <tr>
   	     <td>   	    
            <g:link action="modevent"  controller="newEvent"  class="label" id="${myevent.id}">
            <g:message code="default.edit.label"/>
            </g:link>         	
   	     </td>
   	     <td>
   	      <g:link action="sendtwit"  controller="newEvent"  class="label" id="${myevent.id}">
            <g:message code="default.sendtwit.label"/>
            </g:link>
   	     </td>
   	     <td>  	     
            <g:link action="delevent"  controller="newEvent"  class="label" id="${myevent.id}">
       ...
       <p class = "leftAlign" style="list-style: none outside none;">
          	...
          	<link  class="label" id="${myevent.name}">${myevent.name?.encodeAsHTML()}<link>          
       </p>
       <p class = "leftAlign" style="list-style: none outside none;">
       		 <label  for="eventnamelable" class="label"><g:message code="label.field.user"/>:</label>
          	 <link class="label" id="${myevent.username}">${myevent.username?.encodeAsHTML()}</link>
       </p>
       <p class = "leftAlign" style="list-style: none outside none;">
            <label  for="eventnamelable" class="label" ><g:message code="label.field.description"/>:</label>
          	<label class="label" id="${myevent.description}">${myevent.description?.encodeAsHTML()}</label>
       </p>
       <p class = "leftAlign" style="list-style: none outside none;">   
            <label  for="eventnamelable" class="label" ><g:message code="label.field.date"/>:</label>
            <label  for="eventnamelable" class="label" id="${myevent.sDate}">${myevent.sDate?.encodeAsHTML()}</label>
      </p>
       <p class = "leftAlign" style="list-style: none outside none;">   
     	    <label  for="eventnamelable" class="label" ><g:message code="label.field.time"/>:</label>
            <label class="label" id="${myevent.eventtime}">${myevent.eventtime?.encodeAsHTML()}</label>
       </p>
       <p class = "leftAlign" style="list-style: none outside none;">
            <label  for="eventnamelable" class="label"><g:message code="label.field.place"/>:</label>
          	<label class="label"  id="${myevent.place}">${myevent.place?.encodeAsHTML()}</label>
       </p>
       <p class = "centerAlign" style="list-style: none outside none;"></p>
       	     <div class="leftAlign" id="map" style="width: 400px; height: 300px" >
       	      <script type="text/javascript">
             	  loadMap("${myevent.place}","map");
           	  </script>
      		 </div>
      </div>
  </g:if>
  ...
  <p class="separator"></p>
	 <g:each in="${results}" var="myevent" style="list-style: none outside none;">
	 <table>
	 <tr>
	 <td width="20px" style= "vertical-align: top;">
	 <p>
	  <img class="headLabel" src="${myevent?.user?.profileImg}" width="40px" height="40px"/> 
	 </p>
	 </td>
	 <td>
	 <div class="itemlist" > 	   
       <p class = "leftAlign" style="list-style: none outside none;">
       		<label  for="eventnamelable" class="label"><g:message code="label.field.event"/>:</label>
          	<g:link action="showdetail"  controller="event"  class="label" id="${myevent.id}">${myevent.name?.encodeAsHTML()}</g:link>
       </p>
       <p class = "leftAlign" style="list-style: none outside none;">
       		 <label  for="eventnamelable" class="label"><g:message code="label.field.user"/>:</label>
          	 <link class="label" id="${myevent.username}">${myevent.username?.encodeAsHTML()}</link>
       ...
       <p class = "leftAlign" style="list-style: none outside none;">   
            <label  for="eventnamelable" class="label" ><g:message code="label.field.date"/>:</label>
              <%java.text.SimpleDateFormat dateFormat = new java.text.SimpleDateFormat("dd/MM/yyyy");
				String sDate = dateFormat.format(new Date().parse("yyyy-MM-dd", myevent.eventdate?.toString()))%>
         
            <label class="label"  id="${myevent.eventdate}">${sDate}</label>         
